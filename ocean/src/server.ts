...
export async function getOcean() {
    return fs.readFileSync(process.cwd()+'/OceanQuestion.json')     
}

export async function writeAnswers(answers : {},Name : string) {
    let storedAnswers
    fs.readFile(process.cwd()+'\OceanAnswers.json',"utf8", (err, data) => {
        storedAnswers = JSON.parse(data)
        storedAnswers[Name] = answers
        console.log("written")
        console.dir(storedAnswers)
        console.log(process.cwd()+'\OceanAnswers.json')
        fs.writeFileSync(process.cwd()+'OceanAnswers.json',JSON.stringify(storedAnswers, null, "\t"))
        return {"success":":)"}
    })
