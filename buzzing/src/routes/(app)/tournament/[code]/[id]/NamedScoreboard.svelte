...
<script lang="ts">
    ...
    $: players = Object.values(scores).reduce(
        (acc, s) => {
            ...
            props: {
                ...
                confirmCallback: async (teamNames: Record<string, string>, playerNames: Record<string, string>) => {
                    ...
                    for (const [id, name] of Object.entries(playerNames)) {
                        const teamEntry = Object.values(teamPlayers).find(({ players }) =>
                            Object.keys(players).includes(id)
                        );
                        if (teamEntry) {
            ...
            {#each Object.keys(players) as teamName}
                ...
                {#if teamPlayers}
                    <th colspan={teamPlayers.length + 1} class="team-name"
                        ><span>{teamName}</span>: {sumQuestionScores(teamName)}</th>
                {/if}
            {/each}
        </tr>
        {#if rowArray.length}
