...
export function generateGameToken(
    data: { memberId: string; gameId: string; spectator?: boolean },
...
export async function getDataFromGameToken(
    token: string
): Promise<{ memberId: string; gameId: string; spectator: boolean } | null> {
    try {
        const tokenPayload = jwt.verify(token, privateKey, { algorithms: ["RS256"] }) as jwt.JwtPayload & {
            memberId: string;
            gameId: string;
            spectator: boolean;
        };
