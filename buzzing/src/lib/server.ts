...
export const io = new Server(httpsServer, {
    ...
    allowRequest: async (req, callback) => {
        const gameToken = req.headers.cookie
            ?.split("; ")
            .find((x) => x.split("=")[0] === "gameToken")
            ?.split("=")[1];
...
io.on("connection", async (socket) => {
    ...
    socket.on("buzz", () => {
        ...
        if (
            ...
            (!game.state.currentQuestion.bonus || game.state.currentQuestion.team.players[memberId])
        ) {
    ...
    socket.on(
        ...
        (number: number, playerId: string, teamId: string, category: Category, scoreType: ScoreType | "none") => {
            game.scoreboard.editTossup(number, playerId, teamId, category, scoreType);
            socket.to(gameId).emit("tossupEdit", number, playerId, teamId, category, scoreType);
        }
    );

    socket.on("editBonus", (number: number, teamId: string, scoreType: "correct" | "incorrect" | "none") => {
        game.scoreboard.editBonus(number, teamId, scoreType);
        socket.to(gameId).emit("bonusEdit", number, teamId, scoreType);
    });
    ...
    socket.on("logDump", (data: Omit<Debugger, "socket" | "openWindow">) => {
        try {
            ...
            fs.writeFileSync(process.cwd() + "/debugLogs.json", JSON.stringify([...currentLogs, data], null, "\t"));
        } catch (e) {}
    });
