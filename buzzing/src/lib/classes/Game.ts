...
export class Game {
    ...
    rejoinMember(memberId: string) {
        if (this.leftPlayers[memberId]) {
            ...
            if (team) {
                ...
                const newTeam = new Team(rejoiningPlayerData.team.name, "created", []);
    ...
    scoreQuestion(score: "correct" | "incorrect" | "penalty") {
        ...
        if ((this.state.questionState !== "buzzed" || !this.state.currentBuzzer) && !currentQuestion.bonus) return null;
        ...
        if (bonus) {
            if (score === "correct") {
                this.scoreboard.correctBonus(number, team?.id, currentQuestion.category);
            } else if (score === "incorrect") {
                this.scoreboard.incorrectBonus(number, team?.id, currentQuestion.category);
            }
        } else {
            if (!buzzer) {
                return;
            } else if (score === "correct") {
                this.scoreboard.correctTossup(number, buzzer.id, team?.id, currentQuestion.category);
            } else if (score === "incorrect") {
                this.scoreboard.incorrectTossup(number, buzzer.id, team?.id, currentQuestion.category);
            } else if (score === "penalty") {
                this.scoreboard.penalty(number, buzzer.id, team?.id, currentQuestion.category);
            }
        }
