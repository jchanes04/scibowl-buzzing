        ...
        for (const name of Object.keys(teamCounts)) {
            ...
            if (Object.hasOwn(teamStats, name)) {
                ...
                s.ppg = weightedAvg(
                    s.ppg,
                    s.gamesPlayed,
                    c.correct * pointValues.tossup + c.bonus * pointValues.bonus + c.penalty * pointValues.penalty
                );
                s.accuracy =
                    (s.accuracy * s.buzzes + c.correct) / Math.max(s.buzzes + c.correct + c.incorrect + c.penalty, 1);
                ...
                for (const cat of categories) {
                    ...
                    catS.accuracy =
                        (catS.accuracy * catS.buzzes + catC.correct) /
                    ...
                    categories: Object.fromEntries(
                        categories.map((x) => [
                            x,
                            {
                                ...
                                ppg:
                                    c.categories[x].correct * pointValues.tossup +
                                    c.categories[x].bonus * pointValues.bonus +
                                    c.categories[x].penalty * pointValues.penalty,
                                accuracy:
                                    c.categories[x].correct /
                                    Math.max(
                                        c.categories[x].correct + c.categories[x].incorrect + c.categories[x].penalty,
                                        1
                                    ),
...
function gameStatistics(game: NamedScores) {
    const teamPlayers = Object.values(game).reduce(
        (acc, s) => {
    ...
    const playerList = Object.entries(teamPlayers)
        .map(([teamName, playerNames]) => playerNames.map((x) => generateCombinedKey(x, teamName)))
        .flat();
    const playerCounts: Record<string, QuestionCounts> = Object.fromEntries(
        playerList.map((x) => [x, defaultQuestionCounts()])
    );
    const teamCounts: Record<string, QuestionCounts> = Object.fromEntries(
        Object.keys(teamPlayers).map((x) => [x, defaultQuestionCounts()])
    );
