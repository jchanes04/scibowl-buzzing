...
export function addNamesToScores(game: Game, scores: Scores): NamedScores {
    function getTeamName(teamId: string) {
        const teamData =
            game.teams[teamId]?.data ?? Object.values(game.leftPlayers).find((x) => x.teamID === teamId)?.team;
    ...
    return Object.fromEntries(
        Object.entries(scores).map(([number, pair]) => {
            const namedPair: NamedQuestionPairScore = {
                category: pair.category,
                tossup: Object.fromEntries(
                    Object.entries(pair.tossup).map(([teamId, tossupScore]) => {
                        return [
                            getTeamName(teamId),
                            {
